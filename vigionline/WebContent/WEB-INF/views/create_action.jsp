...
<div class="container">
	...
	<ul class="breadcrumb">
		...
		<li><a href="<%=baseUrl %>/actions"><%=messages.getMessage("actions") %></a>
	...
	<form class="well" action="<%= baseUrl %>/actions/create" method="POST">
		<label for="name"><%= messages.getMessage("name") %></label><input
			name="name" value="${it.action.name }" required="required" /> <label
			for="action1"><%= messages.getMessage("action1") %></label><input
			name="action1" value="${it.action.action1 }" required="required" /> <label
			for="action2"><%= messages.getMessage("action2") %></label><input
			name="action2" value="${it.action.action2 }" /> <label
