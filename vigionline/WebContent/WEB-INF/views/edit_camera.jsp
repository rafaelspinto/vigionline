...
<%@ include file="header.jsp" %>
	<div class="container">
		...
		<ul class="breadcrumb">
			...
			<li><a href="<%=baseUrl %>/cameras/${it.camera.idCamera}">${it.camera.name }</a> <span class="divider">/</span></li>
			<li class="active"><%=messages.getMessage("edit_camera") %></li>
		</ul>
		<!-- End Navigation -->
		<div class="well">
			<form class="well" action="<%= baseUrl %>/cameras/${it.camera.idCamera}/edit" method="POST">
				<input type="hidden" name="idCamera" value="${it.camera.idCamera }" />
				<label for="name"><%= messages.getMessage("name") %></label><input name="name" value="${it.camera.name }" />
				<label for="url"><%= messages.getMessage("url") %></label><input name="url" value="${it.camera.url }" />
				<label for="port"><%= messages.getMessage("port") %></label><input name="port" value="${it.camera.port }" />
				<label for="username"><%= messages.getMessage("username") %></label><input name="username" value="${it.camera.username }" />
				<label for="password"><%= messages.getMessage("password") %></label><input type="password" name="password" value="${it.camera.password }"/>
				<label for="idLocation"><%= messages.getMessage("location") %></label>
				<select id="location" name="idLocation">
